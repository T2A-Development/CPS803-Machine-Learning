...
def create_dictionary(messages):
    ...
    for message in messages:
        word_collection = get_words(message)
        for word in word_collection:
            if word in dict_word:
                ...
                dict_word[word] = 1
    index = 0
    for count in list(dict_word.keys()):
        if dict_word[count] < 5:
            del dict_word[count]
        else:
            dict_word[count] = index
            index += 1
    return dict_word
    # *** END CODE HERE ***
...
def transform_text(messages, word_dictionary):
    ...
    """
    # *** START CODE HERE ***
    word_occurances = np.zeros((len(messages), len(word_dictionary)))
    word_list = list(word_dictionary.keys())
    for message in range(len(messages)):
        normal_message = get_words(messages[message])
        for word in range(len(word_list)):
            occurance = normal_message.count(word_list[word])
            word_occurances[message][word] = occurance
...
def fit_naive_bayes_model(matrix, labels):
    ...
    phi_y1 = (1 + matrix1) / (V + matrix1.sum())
    phi_y0 = (1 + matrix0) / (V + matrix0.sum())
...
def get_top_five_naive_bayes_words(model, dictionary):
    ...
    for i in range(5):
        top_words.extend([list(sorted_dict.keys())[i]])
    return top_words
    # *** END CODE HERE ***
...
def compute_best_svm_radius(train_matrix, train_labels, val_matrix, val_labels, radius_to_consider):
    ...
    for radius in range(len(radius_to_consider)):
        ...
        accMatrix = (svmPredict == val_labels).astype(int)
        dict_accuracy[accMatrix.sum()] = radius_to_consider[radius]
    return dict_accuracy[max(dict_accuracy.keys())]
    # *** END CODE HERE ***
